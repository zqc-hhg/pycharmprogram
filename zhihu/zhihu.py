...
返回：
    无
'''
def get_xml_file(bv_list):
    count = 1

    for av_num in av_list:
        url = 'https://www.bilibili.com/video/bv' + av_num
...
def xml_format(av_num, xml_data):
    ...
    with open(f'output/{bv_num}.xml', 'w', encoding = "utf-8") as f:
        f.write(result)
...
if __name__ == '__main__':
    ...
    print('=============')
    if input_mode == '1':
        av_list = [input('输入视频bv号：\n')]
        print('=============\n')
    elif input_mode == '2':
        with open('av_number.txt') as f:
            bv_list = [x.replace('\n', '') for x in f.readlines()]

    get_xml_file(bv_list)
    print('=============\n')
    print(u'弹幕爬取完成...')
    print(u'2秒后自动退出程序...')
    time.sleep(2)