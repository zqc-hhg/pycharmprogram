...
class WangYiMusicWordCloud:
    ...
    def get_words_text(self):
        if os.path.isfile(f'comment/{self.music_id}.txt'):
            print('评论文件已存在，读取文件...')
            with open(f'comment/{self.music_id}.txt', 'r', encoding='utf-8') as f:
                return f.read()
        else:
            ...
            str_text = '\n'.join(text)
            with open(f'comment/{self.music_id}.txt', 'w', encoding='utf-8') as f:
                f.write(str_text)
...
if __name__ == '__main__':
    ...
    ttf = input("请问用一号字体还是用4号字体:")
    ttf = ttf +'.ttf'
    name = input("文件名称（.jpg）：")
    name = name +'.jpg'
    wordcloud_obj = WangYiMusicWordCloud(music_id, mask='mask.jpg', font_path='4.ttf')
    wordcloud_obj.to_file(name)
